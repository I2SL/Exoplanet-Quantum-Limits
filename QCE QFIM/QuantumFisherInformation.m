% Computes the Quantum Fisher Information Bound for Exoplanet Localization
% under optical axis alignment to the star-planet intensity centroid.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
QFI_rr = (1-kappa.^2)*pi^2 - 4*(1-kappa.^2).*kappa.^2 ...
...
QFI_th = (1-kappa.^2)*pi^2.*r_delta.^2;
...
% Plot QFI radial component curves
figure
hold on
for k = 1:numel(b)
    plot(r_delta/rl,QFI_rr(:,k)./((1-kappa(k).^2)*pi^2),'LineWidth',1,'Color',c1(k,:))
end
...
% QFI radial component
Kappa = 1-2*B;
QFI_rr = (1-Kappa.^2)*pi^2 .* ( 1 - 4.*Kappa.^2 ...
                            .*(besselj(2,2*pi*R)./(pi*R)).^2);
% QFI angular component
QFI_th = (1-Kappa.^2)*pi^2 .* R.^2;
...
Sigma_Loc_rel_normalized = Sigma_Loc_rel .* sqrt(N_ref*(1-Kappa.^2)*pi^2);
